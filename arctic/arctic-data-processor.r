#-- script to process the arctic clinical data and create the cohort
...
readfile <- function(filename){
	read.table(paste0("../received-clinical-datasets/arctic/", filename),
		      na.strings = c("", "NA"),
		      comment.char = "",
		      sep = "|",
		      header = T
...
files <- list.files(path = "../received-clinical-datasets/arctic", pattern = "txt$") 
...
write.table(dims.df, file = "arctic_tabledims.txt", sep = "\t", row.names = F)
...
write.table(cll210.summ, file = "arctic_data_summary.txt", sep = "\t")
...
consent.manifest <- consent.manifest[consent.manifest$Trial == "Arctic",]

#-- read in the file that links TrialNo to PatNo, provided by Fabrizio
participant.manifest <- read.table("../received-clinical-datasets/arctic/trialnos.csv", sep = "|", header = T)
...
#-- columns to EXCLUDE
cols.to.remove <- readLines("arctic-drop-fields.txt")
...
saveRDS(dfs.export, "arctic-research-data.rds")
...
lapply(seq_along(dfs.export), function(i) writefile(dfs.export[[i]], paste0("../research-datasets/arctic/", names(dfs.export)[i])))

#-- CHECKS
#-- get list of exported files
exportedfiles <- list.files("../research-datasets/arctic", full.names = T)
