...
readfile <- function(filename){
	read.table(paste0("../received-clinical-datasets/flair/", filename),
		      na.strings = c("", "NA"),
		      comment.char = "",
		      sep = "|",
		      header = T
...
files <- list.files(path = "../received-clinical-datasets/flair", pattern = "txt$") 
...
#-- write.table(dims.df, file = "flair_tabledims.txt", sep = "\t", row.names = F)
...
#-- write.table(cll210.summ, file = "flair_data_summary.txt", sep = "\t")
...
#-- participant.manifest <- read.table("../received-clinical-datasets/flair/trialnos.csv", sep = "|", header = T)
...
#-- (got valid consent OR are deceased) AND don't have an outstanding consent query 
...
write.table(participant.manifest, "flair-participant-manifest.txt", row.names = F, sep = "\t")

ids.to.include <- participant.manifest$PatNo[participant.manifest$export.to.research]

#-- columns to EXCLUDE
cols.to.remove <- readLines("flair-drop-fields.txt")
...
saveRDS(dfs.export, "flair-research-data.rds")
...
lapply(seq_along(dfs.export), function(i) writefile(dfs.export[[i]], paste0("../research-datasets/flair/", names(dfs.export)[i])))

#-- CHECKS
#-- get list of exported files
exportedfiles <- list.files("../research-datasets/flair", full.names = T)
