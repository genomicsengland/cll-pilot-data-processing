...
readfile <- function(filename){
	read.table(paste0("./data/arctic/", filename),
		      na.strings = c("", "NA"),
		      comment.char = "",
		      sep = "|",
		      header = T
...
files <- list.files(path = "./data/arctic", pattern = "txt$") 
...
participant.manifest <- read.table("./data/arctic/trialnos.csv", sep = "|", header = T)
...
lapply(seq_along(dfs.export), function(i) writefile(dfs.export[[i]], paste0("./researchdata/arctic/", names(dfs.export)[i])))

#-- CHECKS
#-- get list of exported files
exportedfiles <- list.files("./researchdata/arctic", full.names = T)
